...
export const formatDurationInHours = (durationInHours: number): string => {
...
export const formatDurationInSeconds = (durationInSeconds: number): string => {
  const years = Math.floor(durationInSeconds / SECONDS_IN_YEAR);
  const months = Math.floor((durationInSeconds % SECONDS_IN_YEAR) / SECONDS_IN_MONTH);
  const days = Math.floor((durationInSeconds % SECONDS_IN_MONTH) / SECONDS_IN_DAY);
  const hours = Math.floor((durationInSeconds % SECONDS_IN_DAY) / SECONDS_IN_HOUR);
  const minutes = Math.floor((durationInSeconds % SECONDS_IN_HOUR) / SECONDS_IN_MINUTE);
